...
# 모드별 색 — sekaishell/theme.py 의 titlebar_bg·fg 와 맞춘다
VARIANTS = {
    "Sekai": {
    ...
    "Sekai-Light": {
        "BG": {"active": (0xf3, 0xf3, 0xf5), "inactive": (0xe9, 0xe9, 0xec)},
        "FG": {"active": (0x1c, 0x1c, 0x21), "inactive": (0x80, 0x80, 0x88)},
        "EDGE": {"active": (0x39, 0xc5, 0xbb, 0.55), "inactive": (0x00, 0x00, 0x00, 0.10)},
        "HOVER": (0, 0, 0),
    },
}
BG = FG = EDGE = HOVER = None
...
def button(kind, state, look):
    ...
    if look in ("prelight", "pressed"):
        if kind == "close":
            ...
            c.set_source_rgba(*HOVER, 0.10 if look == "prelight" else 0.06)
...
active_text_color=%(fga)s
inactive_text_color=%(fgi)s
...
def main():
    global OUT, BG, FG, EDGE, HOVER
    for name, v in VARIANTS.items():
        OUT = os.path.join(THEMES, name, "xfwm4")
        BG, FG, EDGE, HOVER = v["BG"], v["FG"], v["EDGE"], v["HOVER"]
        one()
...
def one():
    ...
    with open(os.path.join(OUT, "themerc"), "w") as f:
        f.write(THEMERC % {"fga": "#%02x%02x%02x" % FG["active"], "fgi": "#%02x%02x%02x" % FG["inactive"]})
