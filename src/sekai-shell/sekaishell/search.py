...
APP_ALIASES = {
    ...
    "firefox-esr": ("파이어폭스",),
    "firefox": ("파이어폭스",),
    "sekai-files": ("파일 탐색기", "탐색기", "내 PC", "내 컴퓨터", "내 폴더", "폴더", "explorer"),
    "sekai-notepad": ("메모장", "텍스트 편집기", "notepad"),
...
SETTINGS_PAGES = {
    ...
    "sound": ("소리", ["audio-speakers", "audio-volume-high", "multimedia-volume-control",
                     "audio-speakers-symbolic", "audio-volume-high-symbolic"],
              ("sound", "audio", "오디오", "사운드", "볼륨", "음량", "스피커", "마이크")),
    "input": ("키보드 및 마우스", ["input-keyboard", "preferences-desktop-peripherals", "input-keyboard-symbolic"],
              ("키보드", "마우스", "keyboard", "mouse", "입력 장치", "input")),
    "shortcuts": ("단축키", ["preferences-desktop-keyboard-shortcuts", "input-keyboard",
                          "preferences-desktop-keyboard"],
                  ("키보드 단축키", "바로 가기 키", "바로가기 키", "단축 키", "shortcut", "shortcuts",
                   "keyboard shortcut", "hotkey", "핫키", "키 바인딩", "keybind", "키 설정", "키 바꾸기")),
    "multitasking": ("멀티태스킹", ["preferences-system-windows", "view-grid", "window-new"],
                     ("multitasking", "가상 데스크톱", "virtual desktop", "데스크톱", "작업 공간", "workspace",
                      "워크스페이스", "창 전환", "alt tab")),
    ...
    "printers": ("프린터", ["printer", "printer-network", "preferences-devices-printer", "printer-symbolic"],
                 ("printer", "printers", "프린트", "인쇄", "print", "복합기", "cups", "프린터 및 스캐너")),
}
...
SETTINGS_ITEMS = [
    ...
    ("sound", "출력 장치", ("스피커", "speaker", "헤드폰", "headphone", "이어폰", "output", "소리를 재생할 위치",
                        "오디오 장치", "audio device", "기본 출력 장치", "hdmi 소리", "모니터 소리")),
    ("sound", "볼륨", ("음량", "volume", "소리 크기")),
    ("sound", "음소거", ("mute", "소리 끄기")),
    ("sound", "입력 장치", ("마이크", "mic", "microphone", "녹음", "녹음 장치", "input", "기본 입력 장치",
                        "마이크 볼륨", "마이크 음소거")),
    ("sound", "입력 테스트", ("마이크 테스트", "mic test", "마이크 확인")),
    ("sound", "볼륨 믹서", ("믹서", "mixer", "volume mixer", "앱 볼륨", "앱별 볼륨", "앱 소리", "pavucontrol",
                        "앱 출력 장치", "녹음 중인 앱")),
    ("sound", "좌우 밸런스", ("밸런스", "balance", "좌우")),
    ("sound", "프로필", ("profile", "프로파일", "사운드 카드", "sound card", "스테레오", "서라운드", "surround",
                      "hdmi 오디오", "a2dp", "헤드셋 모드")),
    ...
    ("shortcuts", "키보드 단축키 바꾸기", ("단축키", "단축키 변경", "change shortcut", "shortcut", "키 바꾸기",
                                   "hotkey", "핫키", "키 바인딩", "keybind", "바로 가기 키")),
    ("shortcuts", "사용자 지정 단축키 추가", ("단축키 추가", "custom shortcut", "명령 단축키", "앱 실행 단축키",
                                     "나만의 단축키", "add shortcut")),
    ("shortcuts", "단축키 기본값으로", ("단축키 초기화", "reset shortcuts", "단축키 되돌리기")),
    ("shortcuts", "스크린샷 단축키", ("print screen", "prtsc", "캡처 단축키", "win shift s", "화면 캡처 키")),
    ("multitasking", "가상 데스크톱", ("virtual desktop", "데스크톱 이름", "데스크톱 이름 바꾸기", "작업 공간",
                                 "workspace", "워크스페이스", "여러 데스크톱")),
    ("multitasking", "데스크톱 추가", ("새 데스크톱", "new desktop", "데스크톱 만들기", "데스크톱 제거",
                                 "데스크톱 닫기", "win ctrl d")),
    ("multitasking", "작업 표시줄에 표시할 창", ("모든 데스크톱", "현재 데스크톱", "다른 데스크톱의 창",
                                        "taskbar windows", "작업 표시줄 창")),
    ("multitasking", "Alt+Tab 을 누르면 보일 창", ("alt+tab", "alt tab", "알트탭", "창 전환", "window switcher",
                                           "task switcher")),
    ...
    ("locale", "자판 배열", ("한글 자판", "두벌식", "세벌식", "세벌식 최종", "390", "자판")),
    ("locale", "한자 변환 키", ("한자", "hanja", "한자 키", "f9", "한자 변환")),
    ("locale", "처음 입력 상태", ("처음 한글", "기본 입력 상태", "영문으로 시작")),
    ("locale", "단어 단위로 입력", ("단어 단위", "word commit")),
    ("locale", "자동 순서 교정", ("모아치기", "순서 교정", "auto reorder")),
    ("network", "Wi-Fi", ("wifi", "와이파이", "무선", "wireless", "wlan", "무선 네트워크", "무선랜")),
    ("network", "유선 연결", ("이더넷", "ethernet", "랜", "lan", "유선", "wired", "ip 주소", "ip address")),
    ("network", "IP 할당 · DNS 서버", ("고정 ip", "static ip", "수동 ip", "ip 설정", "dhcp", "dns", "dns 서버",
                                  "게이트웨이", "서브넷", "dns over tls")),
    ("network", "VPN", ("vpn", "wireguard", "와이어가드", "openvpn", "vpn 추가", "vpn 연결")),
    ("network", "숨겨진 네트워크 연결", ("숨겨진 네트워크", "hidden network", "ssid")),
    ("network", "회사·학교 Wi-Fi 로그인", ("802.1x", "eap", "peap", "ttls", "eduroam", "엔터프라이즈", "enterprise")),
    ("network", "알려진 네트워크 관리", ("저장된 네트워크", "네트워크 삭제", "저장 안 함", "forget", "와이파이 비밀번호",
                                   "보안 키 보기")),
    ("network", "데이터 통신 연결", ("metered", "데이터 통신", "데이터 절약")),
    ...
    ("bluetooth", "이 PC 의 이름", ("장치 이름", "블루투스 이름", "다른 장치가 찾을 수 있게", "검색 허용",
                             "discoverable")),
    ("printers", "프린터 추가", ("프린터 연결", "add printer", "네트워크 프린터", "무선 프린터", "wifi 프린터",
                            "usb 프린터", "airprint", "ipp everywhere", "프린터 설치", "프린터 드라이버",
                            "ip 주소로 프린터 추가", "원하는 프린터가 목록에 없습니다", "드라이버 설치")),
    ("printers", "프린터 속성", ("printer properties", "프린터 이름 변경", "프린터 공유", "share printer",
                            "드라이버 변경", "프린터 드라이버", "프린터 위치")),
    ("printers", "인쇄 기본 설정", ("printing preferences", "양면 인쇄", "용지 크기", "흑백 인쇄", "컬러 인쇄",
                              "인쇄 품질", "duplex", "a4")),
    ("printers", "기본 프린터", ("default printer", "기본값으로 설정", "기본 프린터 변경")),
    ("printers", "인쇄 대기열", ("print queue", "인쇄 작업", "인쇄 취소", "인쇄 중지", "print job", "스풀러",
                            "spooler", "인쇄가 안 됨")),
    ("printers", "테스트 페이지 인쇄", ("test page", "테스트 인쇄", "인쇄 테스트")),
    ("printers", "프린터 제거", ("remove printer", "delete printer", "프린터 삭제")),
]
